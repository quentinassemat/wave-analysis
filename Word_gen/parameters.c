...
// version dégradée de acceptV pour le test
// pres est le nombre d'octet de précision (min 0, max rV.prec / 8)
#define PREC_V 8 // nombre d'octet de précision

int acceptV(int t, prng_t PRNG) {
	if ((t < rV.min) || (t > rV.max))
		return 0;
	// for (int i = 0; i < rV.prec / 8; ++i) {
	for (int i = 0; i < PREC_V; ++i) {
...
#define PREC_U 8 // nombre d'octet de précision
int acceptU(int t, int l, prng_t PRNG) {
	if ((l < rU[t].min) || (l > rU[t].max))
		return 0;
	for (int i = 0; i < PREC_U; ++i) {

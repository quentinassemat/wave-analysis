...
// t will be always at the mean of the distribution i.e. t = n/2 (1 - (1 - w/n)^2 - 1/2(w/n)^2) - 2/3 (n/2 - k_v') k_v' = k_v - d
// modifié pour avoir une distribution constante autour de la moyenne
int pickV(prng_t PRNG) {
	double wn = (double) w / (double) n;
	double res = 0.5 * ((double) n) * (1 - (1 - wn)*(1 - wn) - 0.5 * wn * wn) - (2.0 / 3.0) * (((double) n) * 0.5 - (kV - d));
	return (int) res;
}
...
//  t = n/2 (1 - (1 - w/n)^2 - 1/2(w/n)^2) - 2/3 (n/2 - k_v') et k_nz = t - l où l = 3w(n-w)/2(n-1)
// modifié pour avoir une distribution constante autour de la moyenne
int pickU(int t, prng_t PRNG) {
	double res = t - (3.0 * (double) w * (double) (n - w)) / (2.0 * (double) (n - 1));
	return (int) res;
}
